...
/*
*y  = scale_factor * (ml - ml0 + 0.5* n * tq * als + n * tq * als * als / 24.0 *
      (5.0 - t + 9.0 * c + 4.0 * SQUARE(c) + als / 30.0 * (61.0 - 58.0 * t
      + SQUARE(t) + 600.0 * c - 330.0 * esp))) + false_northing;
*/

/* This last set of equations split apart to test problems with the newer version of
 * gcc (2.96) which appears to be irretrevable broken.  Although this set of equations
 * are identical to that above (within rounding error), this set generates the right
 * (finite) answer, but the equation above generates NaN from a stable equation and
 * finite variables.
 */

one = 61.0 - 58.0 * t + SQUARE(t) + 600.0 * c - 330.0 * esp;
two = 5.0 - t + 9.0 * c + 4.0 * SQUARE(c) + als / 30.0 * one;
three = 0.5 + als / 24.0 * two;
four = ml - ml0 + n * tq * (als * three);
five = scale_factor * four;
