...
**Branch**: feature/issue-121 at `e1e9be9`
...
### Findings
- [x] (must-fix) Verdict scope not updated after the June bag was demoted — still "every sampled deployment bag" / "all 179,475 messages across both bags" when only one 74-min shoal-water bag of 156 in `bizzyboat_sonar/` is data-of-record; no representativeness or temporal-window limit (Apr bags carry DeltaT not M3; `2026-06-30` has no `sonar_info`) — `.agent/work-plans/issue-121/progress.md:158-172,305-313`
- [x] (must-fix) "Config-verified" citation is dead config — `bizzyboat.yaml:765` uri is overridden at launch by `perception_launch.py:269-278` from `sonar_log_directory` (`:36-41`) + UTC subdir; the yaml path does not exist on disk. Conclusion right, evidence wrong — `.agent/work-plans/issue-121/progress.md:151,249-251,325`, `.agent/work-plans/issue-121/plan.md:92`
- [x] (must-fix) `tx_delays` listed as a bag-supplied observable but is identically 0.0 (same sector object as `tilt_deg`, `node.py:564`) — internally inconsistent with the entry's own first-class `tx_angles` limit — `.agent/work-plans/issue-121/progress.md:202-204`
- [x] (must-fix) Clock-skew limit missing and the `.orig` check was run on the June-09 bag, which predates the Jun-24-2026 skew onset (`retrofit_m3_bag.py:19-22`); the data-of-record Lewes bag sits inside the window and was never checked. Skew is dismissed as payload-irrelevant, but an inversion georeferences beams via odom/TF (1 s at 3 kn ≈ 1.5 m). `--report-only` is a non-destructive probe — `.agent/work-plans/issue-121/progress.md:293-297,305-313`
- [x] (must-fix) ".all = debug by design" recorded as if source-corroborated; `perception_launch.py:114-117` describes the archive as "genuine .all files (Caris/Qimera/MB-System) alongside the sonar bags", configured every deployment — record as operator-stated intent and note the divergence — `.agent/work-plans/issue-121/progress.md:232-243`, `.agent/work-plans/issue-121/plan.md:88-92`
- [x] (suggestion) Report `ping_info.frequency` (= 500000.0 Hz) — the only discriminator ruling out the empty-sector fallback as the cause of all-zero `tx_angles` — `.agent/work-plans/issue-121/progress.md:197-204`
- [x] (suggestion) "nonzero" reported where the plan required "non-zero/non-NaN"; NaN passes `!= 0` and the entry found one in the SV feed — `.agent/work-plans/issue-121/progress.md:174-176`
- [x] (suggestion) "Duration" column is the detections-topic span, unlabeled — June bag runs 97.9 min but the M3 stream stops 17.9 min before the bag ends — `.agent/work-plans/issue-121/progress.md:169-172`
- [x] (suggestion) `Sounding` "stores only" enumeration incomplete — also `depth`, `vertical_error`, `horizontal_error`, `predicted_depth_at_touchdown` (`sounding.h:78-124`) — `.agent/work-plans/issue-121/progress.md:270-271`
- [x] (suggestion) `tx_angles` read-site is `sounding.h:49-50`, not `:52-54` — `.agent/work-plans/issue-121/progress.md:260-262`, `.agent/work-plans/issue-121/plan.md:38`
- [x] (suggestion) "75.46 ms implies ~56 m, deeper than Massabesic" conflates slant range with depth (up to 59.4° off nadir ⇒ as little as ~29 m water) — `.agent/work-plans/issue-121/progress.md:298-301`
- [x] (suggestion) "~118–119° observed swath" assumes port/starboard symmetry from one absolute maximum; "nominal 120°" is uncited — `.agent/work-plans/issue-121/progress.md:206-209`
- [x] (suggestion) "main `bizzyboat/` bags carry no M3 topics" is unsourced — it holds for `bizzyboat/2026-08-05T19-18-17+00-00`; name the bag — `.agent/work-plans/issue-121/progress.md:251-252`
- [x] (suggestion) Q3c omits the favourable corollary — the sonar bag also carries `/bizzy/odom`, `/tf`, `/tf_static` and the SV feed, so no cross-bag join is needed at import — `.agent/work-plans/issue-121/progress.md:244-255`
- [x] (suggestion) State the angle-convention contract — `node.py:565-575` negates the Kongsberg pointing angle (+port → ROS +starboard); mount orientation lives in the URDF — `.agent/work-plans/issue-121/progress.md:197-204`
- [x] (suggestion) Sharpen Q1a/Q3a — `nrx`/`nvalid` are decoded (`em_datagrams.py:152-153`) but never published, so reject counts are unrecoverable; and the dropped-`0x47` claim has empirical backing (1816 × `0x4E` and 1816 × `0x47` in `m3_20260616_155246.all`) — `.agent/work-plans/issue-121/progress.md:186-195,224-231`
- [x] (suggestion) Add a typed entry for the `2900bf1` + `6043cee` pass at `6043cee` — the spike entry's correlation key still reads `eb5aaf7`, which does not contain the operator-corrected text, and the second `## Implementation` entry's Commits list is two commits stale — `.agent/work-plans/issue-121/progress.md:138-155,367-374`
- [x] (suggestion) Hand-typed timestamps contradict git (rewrite claimed 23:45 vs `57d15a8` at 23:40:26; entry `**When**: 23:52` inside 23:40–23:41 commits) and the Local Review is cited "at `707392c`" where its own header gives `eb5aaf7` — `.agent/work-plans/issue-121/progress.md:144-148,340,369-374`
- [x] (suggestion) "Done by host 2026-08-17" is stale — both comments were refreshed 2026-08-18 with the operator corrections — `.agent/work-plans/issue-121/progress.md:326`
- [x] (suggestion) Two entries share the `## Implementation` heading — a last-match consumer gets the remediation log, not the deliverable — `.agent/work-plans/issue-121/progress.md:138,367`
- [x] (suggestion) "`bizzyboat_sonar/` is the data-of-record directory" stated without exception, but the tree also holds `2026-06-04T…_kongsberg_em_test` and `m3_all/` — heuristic, not partition — `.agent/work-plans/issue-121/progress.md:249-252`
- [x] (suggestion) Repo-qualify the ADR-0009 citation (`unh_marine_autonomy ADR-0009`) — bare it collides with workspace ADR-0009 — `.agent/work-plans/issue-121/plan.md:162`
- [x] (suggestion) Plan drift (cosmetic): step 1 still names the un-scanned `bizzyboat/2026-08-03…` bag; the demoted empirical discriminator step carries no inline annotation; Consequences row 2 not updated after the epic signal was posted — `.agent/work-plans/issue-121/plan.md:81-88,105-118`
- [x] (suggestion) Drop the unresolvable "referenced in project memory" phrase (dates are inline) and reconcile "no PR needed" if a PR is opened — `.agent/work-plans/issue-121/progress.md:86`, `.agent/work-plans/issue-121/plan.md:196-198`
- [x] (suggestion) Knowledge-capture candidates, **proposal only — operator decides**: `bizzyboat_sonar/` vs `bizzy_m3/`/`m3_all/` data-of-record distinction; `skip_invalid_beams` default true ⇒ no rejected-beam population in bags; M3's ~38-ping startup sound-speed default. Propose in the PR body; do not edit an instruction file on this branch.
- [x] (owed, host) Must-fix 1-5 also need pushing back into the two published comments (rolker/cube_bathymetry#121 findings comment and the rolker/unh_marine_autonomy#300 epic signal) — the epic currently reads "two deployment recordings". (deferred: host action — this sub-agent performs no `gh` writes; carried as the one open checkbox in the deliverable entry's Findings list, with the specific corrections the refresh must carry)

## Implementation
**Status**: complete
**When**: 2026-08-18 00:34 -04:00
**By**: Claude Code Agent (Claude Opus)

**Branch**: feature/issue-121 at `e1e9be9`
**Addressed**: `## Local Review (Pre-Push)` (2026-08-18 00:20 -04:00, branch head `6043cee`, verdict changes-requested, round 2) — 5 must-fix + 20 suggestions + 1 owed-host item
**Commits**: `4d63366` (evidence scope), `59d4399` (launch-override citation), `de75877` (observable-set consistency + angle convention), `cd6c5f1` (clock-skew probe), `d15da7d` (operator-intent attribution), `9438b15` (Q3/Q4 citations + `.all` census), `e1e9be9` (entry provenance, host-owed sync, plan drift), plus this progress commit (checkbox reconciliation)

**Method**: every new number was measured this round, not carried over. New
read-only measurements: (a) a metadata-only census of all 157 entries in
`~/data/logs/gabby/logs/bizzyboat_sonar/` (152 with `metadata.yaml`; 48 carrying
`/bizzy/sensors/m3/detections`; 14,913,030 detection messages over ~8,657 min;
M3-bearing bags only from 2026-06; 17 with `sonar_info`; 93 with a DeltaT topic);
(b) `retrofit_m3_bag.py --report-only` on the data-of-record Lewes bag —
**non-destructive** (pass 1 measures and returns before any writer opens,
`:506-517`); histogram `+0 s: 124375`, i.e. no #338 clock skew; (c) a `/tf_static`
read of the same bag showing `bizzy/m3` already at the corrected
`(-0.29, 0.0, -0.28)`; (d) a full re-scan of the Lewes detections stream for
`ping_info.frequency` (500000.0 Hz in all 124,375), NaN in `sound_speed` (none),
`tx_delays` (identically 0.0) and the detections-topic span (74.03 min vs 74.04
bag); (e) a datagram census of `m3_all/m3_20260616_155246.all` (1816 each of
`0x4E`/`0x47`/`0x58`, 1817 `0x41`, 65 `0x43`). Source re-reads:
`perception_launch.py:34-41,107-146,275-278`, `bizzyboat.yaml:713-733,765`,
`node.py:202-205,544-545,557-558,564-575`, `em_datagrams.py:100-121,152-153`,
`sounding.h:43-54,70-135`, `retrofit_m3_bag.py:19-22,301-304,506-517`. Bags and
`.all` files were opened read-only throughout; nothing under `~/data` was
modified.

### Actions
- [x] (must-fix) Verdict rescoped to the one data-of-record bag — new "Evidence scope" paragraph quantifies representativeness (0.83% of 14.9 M detection messages; 48 of 152 bags carry M3; June-2026-onward window; 17 with `sonar_info`), separates encode-path claims (generalize) from distributions (do not), and the "179,475 across both bags" aggregate is gone — `progress.md` deliverable entry, Verdict/Q1/Limits
- [x] (must-fix) Dead `bizzyboat.yaml:765` citation replaced with the real mechanism — `perception_launch.py:275-278` overrides `storage.uri` from `$P11_SONAR_LOG_DIR` (`:34-41`) + UTC subdir; the yaml path does not exist on disk — `progress.md` Q3 + header, `plan.md:88-96`
- [x] (must-fix) `tx_delays` no longer listed as an observable — it is filled from the same sector object as the tilt (`node.py:564`) and is identically 0.0; the empty-sector fallback is ruled out by `ping_info.frequency` = 500 kHz (`node.py:544-545,557-558`) — `progress.md` "Observable set"
- [x] (must-fix) Clock skew measured on the data-of-record bag (it sits inside the post-2026-06-24 window): histogram `+0 s: 124375`, no skew; `/tf_static` already corrected; no `.orig` — folded into the secondary observations and a new per-bag precondition in Limits (~1.5 m at 3 kn) — `progress.md` Limits + Secondary
- [x] (must-fix) `.all` debug intent attributed to the operator (Roland, 2026-08-18) with the diverging `perception_launch.py:114-117` "genuine .all files … alongside the sonar bags" comment quoted, plus the driver comment that sides with the operator (`node.py:202-205`); noted that nothing in the verdict turns on the disagreement — `progress.md` Q3, `plan.md:89-97`
- [x] (suggestion) `ping_info.frequency` = 500000.0 Hz reported as the empty-sector discriminator
- [x] (suggestion) "non-zero **and non-NaN**" now reported (the plan's actual test); zero NaNs found in the data-of-record bag's `ping_info.sound_speed`
- [x] (suggestion) Table column relabelled "Detections span" with the bag span alongside (Lewes 74.03 vs 74.04; June 80.1 vs 97.9)
- [x] (suggestion) `Sounding` enumeration completed — `depth`, `vertical_error`, `horizontal_error`, `predicted_depth_at_touchdown` added with line refs, plus the note that `slant_range` is the lossy `twtt·c/2` product
- [x] (suggestion) `tx_angles` read site corrected to `sounding.h:49-50` (rx at `:53-54`, `:70-72`) — `progress.md` Q4, `plan.md:37-39`
- [x] (suggestion) Slant range vs depth disentangled — 75.46 ms is ~56 m of slant range, as little as ~29 m of water at 59.4° off nadir, so it settles nothing about the site
- [x] (suggestion) Swath claim softened — single absolute maxima bound the half-swath, no symmetry assumed, and "nominal 120°" marked as an uncited vendor figure
- [x] (suggestion) "main bags carry no M3 topics" now sourced — the three `bizzyboat/2026-08-05T*` bags of the same day, 60–63 topics each, none matching `m3`
- [x] (suggestion) Q3c's favourable corollary added — the sonar bag carries `/bizzy/odom` (44,360), `/tf` (141,759), `/tf_static` and the SV feed (111,024), so no cross-bag join is needed at import
- [x] (suggestion) Angle-convention contract stated — `node.py:566-575` negates the Kongsberg pointing angle (+port ⇒ ROS +starboard); mount orientation lives in the URDF and rides in the bag's `/tf_static`
- [x] (suggestion) Q1a/Q3a sharpened — `nrx`/`nvalid` decoded but never published (`em_datagrams.py:152-153`), so reject counts are unrecoverable; dropped `0x47` backed by the 1816-per-1816 datagram census
- [x] (suggestion) Deliverable entry re-correlated to `e1e9be9` with a git-derived revision history; the round-1 remediation entry's `Commits` list extended with `2900bf1` + `6043cee`
- [x] (suggestion) Hand-typed times replaced with commit times (round-1 entry now `23:41` at `cb62a2c`; the "rewritten at 23:45 … at `707392c`" line replaced by the revision history)
- [x] (suggestion) "Done by host 2026-08-17" corrected to posted 08-17 / refreshed 08-18
- [x] (suggestion) Shared `## Implementation` heading flagged in the deliverable entry itself with an "Entry role" line telling a last-match consumer that the last entry is a remediation log, not the deliverable (deferred as a *fix*: ADR-0013 has no investigation-result type and minting one is out of scope for a docs-only spike)
- [x] (suggestion) `bizzyboat_sonar/` described as the recorder's target rather than a curated partition — the `kongsberg_em_test` bag, 4 metadata-less directories and `m3_all/` named as exceptions
- [x] (suggestion) ADR-0009 repo-qualified as `unh_marine_autonomy` ADR-0009, distinguished from workspace ADR-0009 — `plan.md:162`
- [x] (suggestion) Plan drift cleared — the un-scanned `2026-08-03` bag annotated with what was scanned instead, the demoted empirical discriminator annotated inline, Consequences row 2 updated with the posted epic signal and the owed refresh
- [x] (suggestion) "referenced in project memory" phrase dropped; "no PR needed" reconciled (docs-only PR still opens per the all-changes-via-PR rule)
- [x] (suggestion) Knowledge-capture candidates recorded as a **proposal only** in the deliverable entry (data-of-record directory + launch override; `skip_invalid_beams` ⇒ no rejected-beam population or count; the ~38-ping startup default) — no instruction file touched on this branch
- [x] (owed, host) Cross-repo comment refresh (deferred: this sub-agent performs no `gh` writes — carried as the single open checkbox in the deliverable entry's Findings, listing exactly what the refresh must carry)
...
- The clock-skew probe is a **net-positive** finding, not a new limit for this
  ...
  post-2026-06-24 bags must be probed individually before an inversion
  georeferences them.

## Local Review (Pre-Push)
**Status**: complete
**When**: 2026-08-18 01:02 -04:00
**By**: Claude Code Agent (Claude Opus)
**Verdict**: changes-requested

**Branch**: feature/issue-121 at `62e24c4`
**Mode**: pre-push
**Depth**: Deep (reason: 885 changed lines; also project-repo trigger `.agent/work-plans/issue-*/plan.md`)
**Must-fix**: 4 | **Suggestions**: 13
**Round**: 3 | **Ship**: recommended — must-fix 8→5→4 and falling; all four are mechanical file:line corrections whose exact replacements are named below, none touches the verdict or its reasoning. Apply them, re-verify only the corrected citations (round 2 replaced one wrong `bizzyboat.yaml` line with another), and push rather than run a full round 4.
**Specialists**: Static analysis (no `.md` linter profile — not checked); Claude Adversarial x2 (Lens A + Lens B, Deep); Governance + Plan Drift; Copilot off (default); Local qwen3.5:35b (no findings; needed `LOCAL_REVIEW_NUM_CTX=40960` — the 885-line diff exceeds the 32768 default)
...
- [ ] (suggestion) "identical for every bag `kongsberg_em_bridge` wrote" is asserted without version evidence, and the entry's own "17 of 48 carry `sonar_info`" is counter-evidence that the driver changed mid-archive. It happens to be true — the twtt/angle encode path settled 2026-06-04 (`a2be7fe`), before any M3-bearing bag; one clause citing that closes the gap — `.agent/work-plans/issue-121/progress.md:199-200`
- [ ] (suggestion) "48 carry `/bizzy/sensors/m3/detections`" — 58 bags *list* the topic, 10 with `message_count: 0`. Say "48 with non-zero counts" so a re-count doesn't read as a discrepancy — `.agent/work-plans/issue-121/progress.md:189`
- [ ] (suggestion) Swath sentence is unparseable and, read literally, wrong: "at most ~118–119° wide and no narrower than the larger side doubled only if symmetric". The supported bound is fan width ≥ max|rx| and ≤ 2·max|rx| (59.1°–118.1°, Lewes) — `.agent/work-plans/issue-121/progress.md:271-272`
- [ ] (suggestion) Importer paths are given relative to the package dir, but the repo root holds both `cube_bathymetry/` and `original_cube/` (which has its own `libsrc/sounding/sounding.h`). Prefix with `cube_bathymetry/` as `plan.md:34` already does — `.agent/work-plans/issue-121/progress.md:358-367`
...
- [ ] (note, no action) Verified clean: scope discipline (docs-only, 885 insertions, no source/test files, no unprompted issue filed, knowledge-capture correctly framed as operator-decided proposals); data-of-record safety (`--report-only` provably opens no writer; nothing under `~/data` modified); ADR-0013 parse (all 8 entries parse, canonical types, zero open checkboxes); no `close`/`fixes`/`resolves` tokens in either file — keep it that way in the PR body, and omit the Test plan section per the docs-only-PR rule
