...
### Findings
- [x] (suggestion) Restart re-stamps all persisted tiles with fresh `prime_version=now()` → warm consumer re-requests the whole store each reboot (broadened blast radius); persist versions or document the trade — `cube_bathymetry_node.cpp:271`
- [x] (suggestion) Drain per-tick budget counts published tiles, not disk loads; empty/absent tiles let one tick issue up to queue-depth `loadWindow` reads — pace by attempts — `cube_bathymetry_node.cpp:882`
